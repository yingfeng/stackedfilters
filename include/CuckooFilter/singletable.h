...
#include "../QuotientFilter/bitsutil.h"
#include "debug.h"
#include "printutil.h"
...
// the most naive table implementation: one huge bit array
    ...
    public:
        ...
        inline uint32_t ReadTag(const size_t i, const size_t j) const {
            ...
            uint128_t bit_mask = ((uint128_t) 1 << bits_per_tag) - 1;
            tag = (*((uint128_t *) p) >> (bits_per_tag * j)) & bit_mask;
        ...
        inline void WriteTag(const size_t i, const size_t j, const uint32_t t) {
            ...
            uint128_t bit_mask = ~((((uint128_t) 1 << bits_per_tag) - 1) << (bits_per_tag * j));
            uint128_t big_tag = ((uint128_t) tag) << (bits_per_tag * j);
            uint128_t existing_bucket = *(uint128_t *) p;
            existing_bucket &= bit_mask;
            existing_bucket |= big_tag;
            ((uint128_t *) p)[0] = existing_bucket;
        }
