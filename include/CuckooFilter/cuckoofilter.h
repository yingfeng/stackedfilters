...
// PackedTable to enable semi-sorting
    ...
    class CuckooFilter {
        ...
        inline void GenerateIndexTagHash(const uint32 &item, size_t *index,
                                         uint32_t *tag) const {
            const uint64_t hash = CityHash64WithSeed((char *) &item, sizeof(uint32), seed_);
    ...
    public:
        explicit CuckooFilter(const size_t max_num_keys, const uint32 seed) : num_items_(0), victim_(), hasher_() {
            seed_ = seed >> 16;
            size_t assoc = 4;
        ...
        Status Add(const uint32 &item);

        // Report if the item is inserted, with false positive rate.
        Status Contain(const uint32 &item) const;

        // Delete an key from the filter
        Status Delete(const uint32 &item);
    ...
    Status CuckooFilter<bits_per_item, TableType, HashFamily>::Add(
            const uint32 &item) {
        size_t i;
        uint32_t tag;
    ...
    template<size_t bits_per_item,
            template<size_t> class TableType, typename HashFamily>
    Status CuckooFilter<bits_per_item, TableType, HashFamily>::AddImpl(
    ...
    template<size_t bits_per_item,
            template<size_t> class TableType, typename HashFamily>
    Status CuckooFilter<bits_per_item, TableType, HashFamily>::Contain(
    ...
    template<size_t bits_per_item,
            template<size_t> class TableType, typename HashFamily>
    Status CuckooFilter<bits_per_item, TableType, HashFamily>::Delete(
            const uint32 &key) {
        size_t i1, i2;
        uint32_t tag;
    ...
    template<size_t bits_per_item,
            template<size_t> class TableType, typename HashFamily>
    std::string CuckooFilter<bits_per_item, TableType, HashFamily>::Info() const {
        std::stringstream ss;
