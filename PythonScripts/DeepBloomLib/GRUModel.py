...
class GRUModel(Model):
    def __init__(self, embeddings_path, embedding_dim, lr=0.005, maxlen=50, pca_embedding_dim=16, batch_size=2056,
                 gru_size=16, hidden_size=8, second_gru_size=None, decay=0.0001, epochs=30, lstm=False,
                 dense_only=False):
    ...
    def fit(self, text_X, text_y):
        ...
        prelayers = [
            Embedding(num_chars + 1, self.embedding_dim if not self.pca_embedding_dim else self.pca_embedding_dim,
    ...
    def predicts(self, text_X):
        text_X.reset_index(inplace=True, drop=True)
        ...
        print("Text Fixed Up")
        with tf.device('/gpu:0'):
            preds = self.model.predict(X, batch_size=16384)
    ...
    def load_model(self, path, text_X, text_y):
        self.model = load_model(path)
        _, _, self.char_indices, self.indices_char = vectorize_dataset(text_X, text_y, self.maxlen)
