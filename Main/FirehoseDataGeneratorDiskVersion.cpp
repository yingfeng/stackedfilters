...
void GenerateDataForOneRun(std::ofstream &file_stream, uint64 negative_universe_size,
    ...
    for (uint32 reps = 0; reps < num_reps; reps++) {
        ...
        printf("Trial FPR:%f, Negative Elements:%ld, sum_int:%ld\n",
               (double) false_positives / (double) sample_size, num_known_negative_elements, sum_int);
    }
    total_fpr /= num_reps;
    used_bits /= num_reps;
...
void set_flags(TCLAP::CmdLine &cmdLine, double &zipf_begin, double &zipf_max, double &zipf_step, double &bits_begin,
               ...
               uint64 &max_known_negatives_max, double &max_known_negatives_ratio, bool &use_hdd,
               bool &allow_caching, uint32 &num_reps, std::string &file_path, int arg_num, char **args) {
        ...
        for (double neg_universe_size = neg_universe_size_begin;
            ...
            for (uint64 max_known_negatives = max_known_negatives_begin;
                ...
                for (double bits_per_positive_element = bits_begin;
                    ...
                    GenerateDataForOneRun(file_stream, neg_universe_size, bits_per_positive_element, use_hdd,
                                          allow_caching, num_reps, max_known_negatives,
                                          negative_sample_size, positives, negatives, cdf, pmf);
                    file_stream.flush();
                }
            }
        }
