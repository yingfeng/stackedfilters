...
namespace {
    ...
    static double SizeFunctionVaried(unsigned num_variables, const double *rank_and_layer_fprs,
        ...
        for (unsigned int i = 0; i < num_layers; i++) {
            if ((i % 2) == 0) {
                size += CuckooFilterLayer<IntElement>::SizeFunction(
    ...
    ThreeLayerCalculateOptimalFPRsGivenPsi(const uint32 num_layers, const size_t total_size, const double fpr,
                                           ...
                                           std::vector<double> &layer_fprs, std::vector<uint32> &fingerprint_length) {
        ...
        for (const auto k1 : possible_bits) {
            double layer_1_fpr = pow(1.0 / 2, k1) * 8;
            uint64 layer_1_size = CuckooFilterLayer<IntElement>::SizeFunction(layer_1_fpr, num_positive);
            ...
            for (const auto k2 : possible_bits) {
                double layer_2_fpr = pow(1.0 / 2, k2) * 8;
                uint64 layer_2_size = CuckooFilterLayer<IntElement>::SizeFunction(layer_2_fpr,
                                                                                  layer_1_fpr * num_negative);
                ...
                for (const auto k3 : possible_bits) {
                    double layer_3_fpr = pow(1.0 / 2, k1) * 8;
                    uint64 layer_3_size = CuckooFilterLayer<IntElement>::SizeFunction(layer_3_fpr,
    ...
    ThreeLayerCalculateOptimalRankAndFPRs(const uint32 num_layers, const size_t total_size, const double fpr,
                                          ...
                                          std::vector<uint32> &fingerprint_length) {
        const double zipf_denominator = approx_zipf_denominator(negative_universe_size, zipf_parameter);
        double min_fpr = 1;
        rank_and_layer_fprs.resize(4);
        const double max_rank = static_cast<double>(max_known_negatives) / negative_universe_size;
        for (double rank = 0.001; rank <= max_rank; rank *= 1.5) {
            const double psi = approx_zipf_cdf(static_cast<uint64>(rank * negative_universe_size), zipf_denominator,
                                               zipf_parameter);
            std::vector<double> temp_layer_fprs;
            std::vector<uint32> temp_fingerprint_length;
            ThreeLayerCalculateOptimalFPRsGivenPsi(num_layers, total_size,
                                                   0, num_positive, psi,
                                                   rank *
                                                   negative_universe_size, temp_layer_fprs, temp_fingerprint_length);
            ...
            if (min_fpr > total_fpr) {
                ...
                fingerprint_length = temp_fingerprint_length;
            }
        }
        if (min_fpr == 1) {
    ...
    GenerateDataForOneRun(std::ofstream &file_stream, const double zipf_parameter, const uint64 negative_universe_size,
        ...
        if (num_layers == 1) {
            ...
            for (const auto k1 : possible_bits) {
                double layer_1_fpr = pow(1.0 / 2, k1) * 8;
                uint64 layer_1_size = CuckooFilterLayer<IntElement>::SizeFunction(layer_1_fpr, num_positive_elements);
                if (layer_1_size >= total_size) {
                    break;
                }
                fingerprint_length[0] = k1;
                rank_and_layer_fprs[1] = layer_1_fpr;
            }

        } else if (num_layers == 3) {
            ThreeLayerCalculateOptimalRankAndFPRs(num_layers, total_size, fpr,
        ...
        for (uint32 reps = 0; reps < num_reps; reps++) {
            ...
            StackedAMQ<CuckooFilterLayer, IntElement> filter(layer_fprs, fingerprint_length, positives,
                                                             known_negatives);
...
int main(int arg_num, char **args) {
    ...
    file_stream
            << "Number Of Positive Elements,Negative Sample Size,Zipf Parameter,Negative Universe Size,Number of Known Negatives Available,Number of Known Negatives Chosen,Psi,Bits Available,Equal Fprs,Num Layers,Used Bits,Total "
               ...
               "Negative\n";
    const uint64 num_positives = 950000;
