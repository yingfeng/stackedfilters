...
void GenerateDataForOneRun(std::ofstream &file_stream, const double zipf_parameter, const uint64 negative_universe_size,
    ...
    for (uint32 reps = 0; reps < num_reps; reps++) {
        ...
        // Build the Stacked Filter with the test data and layer fprs. However, include negative elements in the second
        // layer even if they are merely "close" to being false positives.
        StackedAMQ<BloomFilter, IntElement> filter(layer_fprs, starting_positives, known_negatives, insert_capacity);
        for (int i = 0; i < 3; i++) {
            for(int j = 0; j < filter.layer_array_[i].filter_size_; j++) {
                filter.layer_array_[i].filter_[j] = false;
                filter.layer_array_[i].num_elements_ = 0;
            }
        }
        for(const auto positive : starting_positives) filter.layer_array_[0].InsertElement(positive);

        std::vector<IntElement> negatives_to_be_added;
        for(const auto negative : known_negatives){
            int num_zeros = 0;
            size_t hash_1 = filter.layer_array_[0].getHash1(negative);
            size_t hash_2 = filter.layer_array_[0].getHash2(negative);
            for(int k = 0; k < filter.layer_array_[0].num_hashes_; k++){
                size_t hash = filter.layer_array_[0].getNthHash(hash_1, hash_2, k);
                num_zeros += 1-filter.layer_array_[0].filter_[hash];
            }
            if(num_zeros<=1) negatives_to_be_added.push_back(negative);
        }
        double layer_1_fpr = filter.layer_fprs_[1];
        size_t num_hashes = std::max<size_t>(ceil(-std::log2(layer_1_fpr)), 1);
        size_t layer_1_size = -1. / (std::pow(1. - std::pow(layer_1_fpr, 1. / num_hashes),
                                                  1. / (num_hashes * negatives_to_be_added.size())) - 1.);
        filter.layer_array_[1].filter_.resize(layer_1_size);
        filter.layer_array_[1].filter_size_ = layer_1_size;
        for(const auto negative: negatives_to_be_added) filter.layer_array_[1].InsertElement(negative);

        for(const auto positive: starting_positives){
            if(filter.layer_array_[1].LookupElement(positive)){
                filter.layer_array_[2].InsertElement(positive);
            }
        }
...
void set_flags(TCLAP::CmdLine &cmdLine, double &zipf_begin, double &zipf_max, double &zipf_step, double &bits_begin,
    ...
    TCLAP::ValueArg<uint64> num_positives_arg("", "neg_universe_size",
                                                  "The starting value for the negative universe size, and the only value if neg_universe_size_max is not set.",
                                                  false, 1000000, "Expects a uint64.");
    TCLAP::ValueArg<uint64> num_positives_max_arg("", "neg_universe_size_max",
                                                      "The max value for the negative universe size.",
                                                      false, 10000000000, "Expects a uint64.");
    TCLAP::ValueArg<double> num_positives_ratio_arg("", "neg_universe_size_ratio",
                                                        "The ratio between steps over which [neg_universe_size, neg_universe_size_max) will be explored.",
                                                        false, 100000000, "Expects a double.");
...
int main(int arg_num, char **args) {
    ...
    set_flags(cmd, zipf_begin, zipf_max, zipf_step, bits_begin, bits_max, bits_step, fpr_begin, fpr_min, fpr_ratio,
              ...
              neg_universe_size_ratio, num_positives_begin, num_positives_max, num_positives_ratio, max_known_negatives_begin, max_known_negatives_max, max_known_negatives_ratio,
