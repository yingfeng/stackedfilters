...
std::vector<double> get_pmf(int num_negatives) {
...
static double SizeFunctionVaried(unsigned num_variables, const double *rank_and_layer_fprs,
    ...
    for (unsigned int i = 0; i < num_layers; i++) {
        size_t temp_size;
        if ((i % 2) == 0) {
            temp_size = BloomFilter<IntElement>::SizeFunction(
...
void GenerateDataForOneRun(std::ofstream &file_stream, uint64 negative_universe_size,
                           ...
                           const std::vector<IntElement> &positives, const int num_negatives) {
    double known_fpr = 0;
    ...
    std::vector<IntElement> negatives = get_negatives(num_negatives);
    std::vector<double> pmf = get_pmf(num_negatives);
    ...
    for (uint32 reps = 0; reps < num_reps; reps++) {
        ...
        for (uint64 i = num_known_negative_elements; i < num_negatives; i++) {
            unknown_false_positives += filter.LookupElement(negatives[i]);
        }

        known_fpr += (double) (known_false_positives) /
                     (double) (num_known_negative_elements);
        unknown_fpr +=
                (double) (unknown_false_positives) / (double) (num_negatives - num_known_negative_elements);
...
int main(int arg_num, char **args) {
    ...
    file_stream
            << "Number Of Positive Elements,Sample Size,Positive Rate,Negative Universe Size,Number of Known Negatives Available,Number of Known Negatives Chosen,Psi,Bits Available,Equal Fprs,Num Layers,Used Bits,Total "
               "FPR,Known FPR,Unknown FPR,EFPB\n";
                    ...
                    if (fpr_begin < 0) {
                        ...
                        for (double fpr = fpr_begin;
                            ...
                            GenerateDataForOneRun(file_stream,
                                                  ...
                                                  equal_fprs, use_hdd, allow_caching,
                                                  num_layers, num_reps, max_known_negatives, num_positives,
                                                  negative_sample_size, positive_rate, positives, neg_universe_size);
                            file_stream.flush();
                        }
